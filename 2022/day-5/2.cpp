...
to move elements -> slice list to move from from bottom and bottom - n crates to carry
-> save to and array and remove elem from vector
append sliced list to crate to append to
*/
...
int main() {
    ...
    if (myfile.is_open()) {
        while ( getline (myfile,line) ) {
            ...
            // get sliced array to move to list
            vector<string> cratesToMove(
                board[crateToMoveFrom].end() - nCratesToMove, board[crateToMoveFrom].end()
            );

            // add sliced array to board
            board[crateToMoveTo].insert(
                board[crateToMoveTo].end(), cratesToMove.begin(), cratesToMove.end()
            );

            // remove n elements from bottom of board
            board[crateToMoveFrom].erase(
                board[crateToMoveFrom].end() - nCratesToMove, board[crateToMoveFrom].end()
            );
